...
###########Description###########
#This code performs a permutation test based on the the t-statistic from Welch's t-test and produces a histogram of the observed test statistic, 
...
perm_test_welch_t <- function(file_path_and_name, score_col_name, group_col_name, nreps){
  #Libraries
  library(foreach)
  library(doParallel)
  library(iterators)
  ...
  n.i <- as.vector(table(data_scores))   # Create vector of sample sizes
  k <- length(n.i)
  
  init_model <- t.test(data_scores[1:n.i[1]], data_scores[(n.i[1] + 1):(n.i[1] + n.i[2])], var.equal = FALSE)
  ...
  t_vals<-foreach(i = 1:nreps, .combine='c') %dopar% {
    ...
    obs_t <-(t.test(newScore[1:n.i[1]], newScore[(n.i[1] + 1):(n.i[1] + n.i[2])], var.equal = FALSE))$statistic
  }
  ...
  par(mfrow = c(2,1))
  hist(t_vals, breaks = 50, main = "Histogram of (Welch's) observed t-statistic on Randomized Samples",
